...
def dismatcreate(POS):
    ...
    for AtomInd1, Pnt1 in enumerate(POS['LattPnt']):
        ...
        for AtomInd2, Pnt2 in enumerate(POS['LattPnt']):
            ...
            PntDis2 = Pnt1-Pnt2
            for i in range(3):
                if (PntDis[i] > 1):#POS['Base'][i][i]):
                    PntDis[i] = 2 - PntDis[i]#* POS['Base'][i][i] - PntDis[i]
                elif (PntDis[i] < -1):#POS['Base'][i][i]):
                    PntDis[i] = PntDis[i] + 2# * POS['Base'][i][i]
                elif (PntDis[i] >= -1) & (PntDis[i] <= 1):#['Base'][i][i]) & (PntDis[i] <= POS['Base'][i][i]):
                    PntDis[i] = PntDis[i]
                else:
                    print("Something is wrong when calculating dist matrix")

            PntDis = np.dot(np.transpose(PntDis), POS['Base'])
            PntDis2 = np.dot(PntDis2, POS['Base'])
            # PntDis = np.dot(np.transpose(PntDis), POS['Base'])#.dot(np.transpose(pos['LattPnt'][i]), pos['Base'])
            POS['dismat'][AtomInd1][AtomInd2] = math.sqrt(np.power(PntDis[0], 2) + np.power(PntDis[1], 2) + np.power(PntDis[2], 2))
            PntDis2_list.append(math.sqrt(np.power(PntDis2[0], 2) + np.power(PntDis2[1], 2) + np.power(PntDis2[2], 2)))
    return POS
##########