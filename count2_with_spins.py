...
def apply_basis_vects(pos_list):
    for pos in pos_list:
        for i in range(len(pos['LattPnt'])):
            pos['LattPnt'][i] = np.dot(np.transpose(pos['LattPnt'][i]), pos['Base'])
...
def count_clusters(data_file, cluster_file, clust_out, eng_out, counts_out, comps_out, vols_out, names, tol, dim):
    ...
    for i in range(len(data)):
        if "#" in data[i]: # "#" indicates new DFT data point
            ...
            element_number = len(species)
            name = set_data[element_number]
            atom_numb = [int(set_data[i]) for i in range(element_number)]
            atom_sum = sum(atom_numb) # Total number of atoms in each DFT data point
            lat_true = [float(set_data[element_number+2]), float(set_data[element_number+3]), float(set_data[element_number+4])]
            lat_ang = [float(set_data[element_number+5]), float(set_data[element_number+6]), float(set_data[element_number+7])]
            base = [data[i+2+j].split() for j in range(3)]
            base = [[float(base[j][k]) for k in range(len(base[j]))] for j in range(len(base))]
            element_name = data[i].split()
            element_name.pop(0)
            issel = 0
            latType = 'Direct'
            # Scale energy to per atom
            enrgs.append(float(set_data[element_number+1]) / atom_sum)
            pos_list = [] # list of positions for each atom
            spin_list = [] # list of spin at each atom
            type_list = [] # list of species of each atom
            comp_list.append(atom_numb) # number of each atomic species
            for j in range(int(atom_sum)): # fill the above three lists
                ...
                atom_pos = line #np.dot(np.transpose(line), base)
                pos_list.append(atom_pos)
                spin_list.append(spin)
                type_list.append(atom_type)
            # Fill dictionary pos
            pos['CellName'] = name
            pos['LattConst'] = lat_true + lat_ang
            pos['LattTrue'] = lat_true
            pos['UnitVolume'] = lat_true[0]*lat_true[1]*lat_true[2]*np.sqrt(1-np.power(np.cos(lat_ang[0]),2)-np.power(np.cos(lat_ang[1]),2)-np.power(np.cos(lat_ang[2]),2)+2*np.cos(lat_ang[0])*np.cos(lat_ang[1])*np.cos(lat_ang[2]))/atom_sum
            pos['Base'] = base
    ...
    newPOS = celib.createSupercell(pos_old_list, dim) # Create copy of dictionary for supercell creation (this is needed to
    ...
    apply_basis_vects(newPOS)
    # count numbers of each cluster
    cluster_count = []
    print("\nCounting clusters on each set..")
    for i in range(len(newPOS[0]['dismat'][0])):
        print(str(newPOS[0]['LattPnt'][i]) + " " + str(newPOS[0]['dismat'][0][i]) + "\n")
    for i in range(len(newPOS)):
        pos_pt = newPOS[i]
        names.append(newPOS[i]['CellName'])
        # These are the two functions that count the instances of each cluster for a particular DFT data point
        cl_list = celib.clustercount1(cluster, pos_pt, TS=tol)
        # fig = plt.figure()
    ...
    # If you have duplicate data in your folders (or degenerate cluster counts) talk to Brian and He will tell you what to do
...
def save_data(output_file, clusters, energies, counts, comps, names):
...
def read_data(input_file, cluster_file, clusters, energies, counts, comps, names):
    ...
    for line in lines:
        if "Clusters" not in line and "Counts" not in line and fill_counts == False:
            ...
            names.append(line[0])
            c1 = line[1]
            comps.append([float(line[1].strip(' [')), float(line[2]), float(line[3].strip(']'))])
